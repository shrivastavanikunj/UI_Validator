        ...
        function handleKeyDown(e) {
            ...
            if (!handled) {
                if (!e.shiftKey && !e.altKey) {
                    ...
                    if (e.which == 36) { //keyCode.HOME
                        handled = (e.ctrlKey) ? navigateTop() : navigateRowStart();
                    } else if (e.which == 35) { //keyCode.END
            ...
            if (!handled) {
                if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
                    ...
                    if (e.which == 27) { //keyCode.ESCAPE
                        if (!getEditorLock().isActive()) {
                            return; // no editing mode to cancel, allow bubbling and default processing (exit without cancelling the event)
                        }
                        cancelEditAndSetFocus();
                    } else if (e.which == 34) { //keyCode.PAGE_DOWN
                        navigatePageDown();
                        handled = true;
                    } else if (e.which == 33) { //keyCode.PAGE_UP
                        navigatePageUp();
                        handled = true;
                    } else if (e.which == 37) { //keyCode.LEFT
                        handled = navigateLeft();
                    } else if (e.which == 39) { //keyCode.RIGHT
                        handled = navigateRight();
                    } else if (e.which == 38) { //keyCode.UP
                        handled = navigateUp();
                    } else if (e.which == 40) { //keyCode.DOWN
                        handled = navigateDown();
                    } else if (e.which == 9) { //keyCode.TAB
                        handled = navigateNext();
                    } else if (e.which == 13) { //keyCode.ENTER
                        if (options.editable) {
                            if (currentEditor) {
                                // adding new row
